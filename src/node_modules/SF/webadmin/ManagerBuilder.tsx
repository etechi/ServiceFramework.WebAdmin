...
function buildEntityItem(ctx: IBuildContext) :void{
    ...
    if (!readonly) {
        if (CreateAction && (ctx.item.entityMode != EntityItemMode.NoCreate))
            chdRoutes.push(<Route key="new" path="new" component={Views.EditView({
                    links: [{ text: ctx.module.title }, { text: title }, { text: '添加' + entity }],
                    controller: controller.Name,
                    serviceId: ctx.item.service,
                })}/>);
        if (UpdateAction)
            chdRoutes.push(<Route key="edit" path="edit/*" component={Views.EditView({
    ...
    if (ListAction || QueryAction) {
        ...
        entityActions.filter(a => a.type == ActionType.ContextForm || a.type == ActionType.GlobalForm).forEach(action => {
            ...
            chdRoutes.push(<Route path={action.action.Name + "/*"} component={Views.EditView({
    ...
    ctx.routes.push(
        <Route path={entityEncoded} component={ListAction || QueryAction ? Views.ListView({
...
function buildFormItem(ctx: IBuildContext):void {
    ...
    ctx.routes.push(<Route path={actionName} component={Views.EditView({
            id:1,
            links: [{ text: ctx.module.title }, { text: title }],
            controller: controller.Name,
            serviceId: ctx.item.service,
...
function buildListItem(ctx: IBuildContext):void {
    ...
    var chdRoutes: JSX.Element[] = [];
    if (loadAction)
        chdRoutes.push(<Route path= "detail/*" component={Views.EditView({
    ...
    if (!readonly) {
        if (createAction)
            chdRoutes.push(<Route path="new" component={Views.EditView({
        ...
        if (saveAction)
            chdRoutes.push(<Route path="edit/*" component={Views.EditView({
    ...
    ctx.routes.push(<div><Route path={actionName} component={Views.ListView({
...
function buildHelpItem(ctx: IBuildContext) {
    ...
    ctx.routes.push(<Route path="help" component={Views.HelpView({
...
function buildIFrameItem(ctx: IBuildContext) {
    ...
    ctx.routes.push(<Route path={encodeURI(ctx.item.title)} component={Views.IFrameView({
...
function buildModule(ctx: IBuildContext) {
    ...
    if (ctx.routes.length > 0)
        routes.push(<Route path={ moduleEncoded}>{ctx.routes}</Route>);
    ctx.routes = routes;
