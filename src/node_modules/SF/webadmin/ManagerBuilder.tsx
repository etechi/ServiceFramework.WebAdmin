...
function buildEntityItem(ctx: IBuildContext) :void{
    ...
    if (LoadForEditAction || GetAction || LoadAction)
    {
        ...
        chdRoutes.push(
            <Route key="detail" path="detail/*" component={view}/>
        );
    }
    if (!readonly) {
        if (CreateAction && (ctx.item.entityMode != EntityItemMode.NoCreate))
            chdRoutes.push(<Route key={entity+"-new"} path="new" component={Views.EditView({
                    links: [{ text: ctx.module.title }, { text: title }, { text: '添加' + entity }],
                    controller: controller.Name,
                    serviceId: ctx.item.service,
                })}/>);
        if (UpdateAction)
            chdRoutes.push(<Route key={entity+"-edit"} path="edit/*" component={Views.EditView({
    ...
    if (ListAction || QueryAction) {
        ...
        entityActions.filter(a => a.type == ActionType.ContextForm || a.type == ActionType.GlobalForm).forEach(action => {
            ...
            chdRoutes.push(<Route key={entity+"-"+action.action.Name} path={action.action.Name + "/*"} component={Views.EditView({
    ...
    ctx.routes.push(
        <Route exact key={entityEncoded} path={entityEncoded} component={ListAction || QueryAction ? Views.ListView({
    ...
    ctx.routes.push.apply(ctx.routes,chdRoutes);
}
...
function buildFormItem(ctx: IBuildContext):void {
    ...
    ctx.routes.push(<Route key={actionName} path={actionName} component={Views.EditView({
            id:1,
            links: [{ text: ctx.module.title }, { text: title }],
            controller: controller.Name,
            serviceId: ctx.item.service,
...
function buildListItem(ctx: IBuildContext):void {
    ...
    var chdRoutes: JSX.Element[] = [];
    if (loadAction)
        chdRoutes.push(<Route key={entity+"-detaul"} path= "detail/*" component={Views.EditView({
    ...
    ctx.routes.push(<Route exact key={actionName} path={actionName} component={Views.ListView({
...
function buildHelpItem(ctx: IBuildContext) {
    ...
    ctx.routes.push(<Route key="help" path="help" component={Views.HelpView({
...
function buildIFrameItem(ctx: IBuildContext) {
    ...
    ctx.routes.push(<Route key={encodeURI(ctx.item.title)}  path={encodeURI(ctx.item.title)} component={Views.IFrameView({
...
function buildModule(ctx: IBuildContext) {
    ...
    if (ctx.routes.length > 0)
        routes.push(<Route key={ moduleEncoded} path={ moduleEncoded}><div>{ctx.routes}</div></Route>);
    ctx.routes = routes;
